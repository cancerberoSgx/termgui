...
def main
  ...
  ed = screen.append_child Editor.new(value: 'Welcome to this\nhumble editor', x: 0.2, width: 0.5, height: 0.5, y: 0.2, cursor_y: 0, cursor_x: 0, style: Style.new(border: Border.new,  padding: Bounds.new(top: 1, left: 2)  ))
...
class Editor < TermGui::Element
  ...
  def initialize(**args)
    ...
    on('enter'){
      log 'enter'
      unless @editor
        @editor = TermGui::EditorBase.new(managed: true, text: @args[:value] || @text || '', screen: root_screen, 
